...
\item {\bf Limitations}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: The Discussion section identifies two limitations: (1) the number of prototypes $K$ must be specified in advance, which may not be optimal across datasets and depends on factors such as data quality and scale; and (2) downstream performance depends on the quality of the affinity graph, as noisy or poorly constructed affinities can propagate misleading community structure into the learned prototypes.
...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: All quantitative results in Tables~1 and~2 are reported as mean~$\pm$~standard deviation over the natural evaluation units: modularity, coverage, and homogeneity are averaged across batches; batch entropy and purity are averaged across metacells. The source of variability is explicitly stated in each table caption. For the spatial experiments, statistical significance of purity comparisons in Fig.~\ref{fig:spatial_tradeoff} is assessed using one-sided Mann-Whitney U tests against scProto, with significance levels reported at $p{<}0.05$, $p{<}0.01$, and $p{<}0.001$.

\item {\bf Experiments compute resources}
    \item[] Question: For each experiment, does the paper provide sufficient information on the computer resources (type of compute workers, memory, time of execution) needed to reproduce the experiments?
    \item[] Answer: \answerYes{}
    \item[] Justification: All experiments were run on a single NVIDIA A100 GPU (40\,GB HBM2) via Google Colab. Training is lightweight: Stage~1 pretraining (50 epochs) and Stage~2 joint prototype learning with early stopping on modularity together complete in well under an hour per dataset.
...
\item {\bf Licenses for existing assets}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: All datasets and baseline methods are cited in the paper. The datasets used (Pancreas, PBMC/Immune, Lung, NSCLC) are publicly available research datasets and their original papers are cited. All baseline methods are similarly cited with references to their original publications.
