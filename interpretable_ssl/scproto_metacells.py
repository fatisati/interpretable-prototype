...
def extract_proto_labels(
    ...
    if similarity == "v12":
        for p in range(sample_proto_sim.shape[1]):
            w = softmax(sample_proto_sim[:, p] / epsilon)  # softmax over cells

            for k in label_keys:
                score = {}
                for lab, s in zip(labels[k], w):
                    score[lab] = score.get(lab, 0.0) + s

                proto_to_label[k].append(
                    max(score, key=score.get) if score else None
                )
