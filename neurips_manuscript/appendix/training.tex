...
The three scRNA-seq datasets (Pancreas, PBMC, Lung) are taken from the benchmarking collection of \citet{luecken2020benchmarking}.
...
By stopping the gradient, the encoder receives no gradient from $\mathcal{L}_{\text{rec}}$, preventing reconstruction from conflicting with the affinity-based objectives; the prototypes are jointly regularised by all objectives: they must sit at the centre of well-connected cell communities \emph{and} be decodable to meaningful gene-expression profiles.
